...
GREEDYpath_cost = 0; % cost g(n): start node to the current node n
goal_distance = distance(xNode, yNode, xTarget, yTarget); % cost h(n): heuristic cost of n
QUEUE(QUEUE_COUNT, :) = insert(xNode, yNode, xNode, yNode, GREEDYpath_cost, goal_distance, goal_distance);
QUEUE(QUEUE_COUNT, 1) = 0; % What does this do?   Reinitalize to 0,then it is explored.


GREEDYtotalexpand=0;
GREEDYtotaldiscover=0;
%% Start the search
while((xNode ~= xTarget || yNode ~= yTarget) && NoPath == 1)
    % expand the current node to obtain child nodes
    exp = expand(xNode, yNode, GREEDYpath_cost, xTarget, yTarget, OBSTACLE, MAX_X, MAX_Y);
    exp_count  = size(exp, 1);
    GREEDYtotaldiscover=GREEDYtotaldiscover+exp_count;
    GREEDYtotalexpand=GREEDYtotalexpand+1;
...
if ((xval == xTarget) && (yval == yTarget))
    ...
    uiwait(h, 5);
     ...
     costs = [GREEDYtotalexpand GREEDYtotaldiscover  GREEDYpath_cost];
